...
if [ -z "$JDK" ] && [ -s "$HOME/.PyCharm2019.2/config/pycharm.jdk" ]; then
  USER_JRE=$("$CAT" "$HOME/.PyCharm2019.2/config/pycharm.jdk")
  if [ ! -d "$USER_JRE" ]; then
...
elif [ -r "$HOME/.PyCharm2019.2/config/pycharm$BITS.vmoptions" ]; then
  # user-overridden
  VM_OPTIONS_FILE="$HOME/.PyCharm2019.2/config/pycharm$BITS.vmoptions"
...
"$JAVA_BIN" \
  ...
  -Didea.paths.selector=PyCharm2019.2 \
